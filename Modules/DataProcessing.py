...
class DataProcessing():
    ...
    def cleanDF(acceptable_array, column_name, data_frame):
        comparator = list(
            map(lambda element: element in acceptable_array, data_frame[column_name]))
    ...
    def parallel(full_data_frame, pdata_frame, country, cereal_dictionary, column_name):
        ...
        for cereal in cereal_dictionary.keys():
            ...
            try:
                avgYield = key_df[key_df["Item"] ==
                                  cereal]["Average Yield"].iloc[0]
            except:
                avgYield = 0
    ...
    def categorizeStrings(Table, targetString):
        ...
        for column in Table.columns:
            if type(Table[column][0]) == str:
                ...
                Table = Table.join(pd.get_dummies(
    ...
    def prune_index(inner_tree, index, threshold):
        ...
        if inner_tree.children_left[index] != TREE_LEAF:
            DataProcessing.prune_index(
                inner_tree, inner_tree.children_left[index], threshold)
            DataProcessing.prune_index(
                inner_tree, inner_tree.children_right[index], threshold)
